...
pub trait VersionStore: Send + Sync {
    ...
    async fn evolution(
        &self, uri: &ContextUri,
    ) -> Result<Vec<crate::model::TemporalVersion>>;
...
pub trait CommitOps: Send + Sync {
    async fn commit(&self, scope: &ContextUri, changes: ChangeSet, meta: CommitMeta) -> Result<CommitId>;
    async fn log(&self, scope: &ContextUri, opts: &LogOpts) -> Result<Vec<Commit>>;
}
...
pub trait BranchOps: Send + Sync {
    async fn create_branch(&self, scope: &ContextUri, name: BranchName, from: CommitId, bt: BranchType) -> Result<Branch>;
    async fn list_branches(&self, scope: &ContextUri) -> Result<Vec<Branch>>;
    async fn delete_branch(&self, scope: &ContextUri, name: &BranchName) -> Result<()>;
...
pub trait HistoryOps: Send + Sync {
    async fn read_at(&self, uri: &ContextUri, ref_: VersionRef, level: ContentLevel) -> Result<ContentPayload>;
    async fn asof_read(&self, uri: &ContextUri, when: AsOfTime, level: ContentLevel) -> Result<ContentPayload>;
    async fn provenance(&self, uri: &ContextUri) -> Result<ProvenanceGraph>;
    async fn impact_analysis(&self, commit: &CommitId) -> Result<ImpactAnalysis>;
    async fn evolution(&self, uri: &ContextUri) -> Result<Vec<crate::model::TemporalVersion>>;
    async fn semantic_diff(&self, scope: &ContextUri, a: &CommitId, b: &CommitId) -> Result<crate::model::StructuredDiff>;
    async fn gc(&self, scope: &ContextUri, policy: &GcPolicy) -> Result<GcReport>;
}
...
impl<T: VersionStore + Send + Sync> CommitOps for T {
    async fn commit(&self, s: &ContextUri, c: ChangeSet, m: CommitMeta) -> Result<CommitId> { VersionStore::commit(self, s, c, m).await }
    async fn log(&self, s: &ContextUri, o: &LogOpts) -> Result<Vec<Commit>> { VersionStore::log(self, s, o).await }
}
#[async_trait]
impl<T: VersionStore + Send + Sync> BranchOps for T {
    async fn create_branch(&self, s: &ContextUri, n: BranchName, f: CommitId, bt: BranchType) -> Result<Branch> { VersionStore::create_branch(self, s, n, f, bt).await }
    async fn list_branches(&self, s: &ContextUri) -> Result<Vec<Branch>> { VersionStore::list_branches(self, s).await }
    async fn delete_branch(&self, s: &ContextUri, n: &BranchName) -> Result<()> { VersionStore::delete_branch(self, s, n).await }
    async fn switch_head(&self, s: &ContextUri, b: &BranchName) -> Result<()> { VersionStore::switch_head(self, s, b).await }
}
#[async_trait]
impl<T: VersionStore + Send + Sync> TagOps for T {
    async fn create_tag(&self, s: &ContextUri, t: Tag) -> Result<()> { VersionStore::create_tag(self, s, t).await }
    async fn list_tags(&self, s: &ContextUri) -> Result<Vec<Tag>> { VersionStore::list_tags(self, s).await }
}
#[async_trait]
...
impl<T: VersionStore + Send + Sync> HistoryOps for T {
    async fn read_at(&self, u: &ContextUri, r: VersionRef, l: ContentLevel) -> Result<ContentPayload> { VersionStore::read_at(self, u, r, l).await }
    async fn asof_read(&self, u: &ContextUri, w: AsOfTime, l: ContentLevel) -> Result<ContentPayload> { VersionStore::asof_read(self, u, w, l).await }
    async fn provenance(&self, u: &ContextUri) -> Result<ProvenanceGraph> { VersionStore::provenance(self, u).await }
    async fn impact_analysis(&self, c: &CommitId) -> Result<ImpactAnalysis> { VersionStore::impact_analysis(self, c).await }
    async fn evolution(&self, u: &ContextUri) -> Result<Vec<crate::model::TemporalVersion>> { VersionStore::evolution(self, u).await }
    async fn semantic_diff(&self, s: &ContextUri, a: &CommitId, b: &CommitId) -> Result<crate::model::StructuredDiff> { VersionStore::semantic_diff(self, s, a, b).await }
    async fn gc(&self, s: &ContextUri, p: &GcPolicy) -> Result<GcReport> { VersionStore::gc(self, s, p).await }
}
