...
impl CognitiveSelfPlay {
    pub fn new() -> Self { Self { policy: PolicyModule::new(), value: ValueModule::new() } }

    /// 执行 N 轮自我对弈，产生偏好对。
    pub async fn self_train(&self, rounds: usize, initial_state: &CognitiveState) -> Vec<SelfPlayResult> {
        ...
        for _ in 0..rounds {
            ...
            let mut valued: Vec<_> = candidates.iter().map(|c| {
            ...
            results.push(SelfPlayResult {
                preference_pair: CognitivePreferencePair {
                    chosen: crate::TrajectorySummary { task_id: "self-play".into(), task_description: best_c.description.clone(), success: true, steps: 1, contradictions: 0, avg_confidence: best_c.confidence },
                    rejected: crate::TrajectorySummary { task_id: "self-play".into(), task_description: worst_c.description.clone(), success: false, steps: 2, contradictions: 1, avg_confidence: worst_c.confidence },
                    preference_source: PreferenceSource::KnowledgeConsistency,
                    confidence: best_v.composite,
                    cognitive_delta: CognitiveDelta { contradiction_diff: -1, confidence_diff: best_v.composite - worst_v.composite, evidence_diff: 0, knowledge_graph_growth: 0 },
                },
