...
//! 联邦查询通过 EventBus broadcast + 各 shard 返回匹配结果。

use crate::marketplace::types::*;
use agent_context_db_core::{ContentType, ContextUri, EventBus, Result, VectorIndex};
...
impl FederatedRegistry {
    ...
    pub fn publish(&self, entry: MarketEntry) {
        // 注册到向量索引
        let uri = format!("market://{}/{}", entry.publisher, entry.id.0);
        ...
        if let Some(ref bus) = self.event_bus {
            let topic = format!("market.publish.{}", entry.domain);
            if let Ok(json) = serde_json::to_vec(&entry) {
                tokio::spawn(async move { let _ = bus.publish(&topic, &json).await; });
            }
        }
    }
