...
impl CognitiveTrainingPipeline {
    ...
    pub async fn train(
        ...
        let mut report = TrainingReport { epochs: vec![], accuracy_delta: 0.0 };
        ...
        for epoch in 0..config.epochs {
            ...
            for pref in &preferences {
                ...
                let grad = CognitiveGradient {
                    source_uri: ContextUri::parse(&format!(
                        "uwu://t/a/x/skill/{}", &pref.chosen.task_id
                    ))
                    .unwrap_or_else(|_| {
                        ContextUri::parse("uwu://t/a/x/skill/fallback").unwrap()
                    }),
                    epistemic_type: ct,
            ...
            all_gradients.sort_by(|a, b| b.weight.partial_cmp(&a.weight).unwrap_or(std::cmp::Ordering::Equal));
            ...
            let accuracy = if gradients_applied > 0 {
                // 准确率 = 采纳梯度数 / 总梯度数
                let base = gradients_applied as f32 / (gradients_applied + gradients_rejected) as f32;
                // 惩罚矛盾
    ...
    pub async fn train_from_trajectories(
        ...
        for epoch in 0..config.epochs {
            ...
            tracing::info!(epoch = epoch, count = products.len(), "consolidation complete");

            // ── 阶段 3: 认知梯度提取 ──
            let gradients = crate::extract_gradients_from_products(&products, config.min_confidence);
            tracing::info!(epoch = epoch, gradients = gradients.len(), "gradients extracted");
                ...
                if let Some((chosen, rejected)) = find_best_worst_pair_from_gradients(&gradients) {
                    ...
                    if loss < 0.0 {
                        ...
                        if let ContentType::Skill = gradient.epistemic_type {
                            let skill_embedding = self.llm.embed(&gradient.source_uri.to_string())
                                .await
                                .unwrap_or_else(|_| vec![0.0_f32; 1536]);
    ...
    pub fn evaluate_gate(
        ...
        let new_wins = report
            ...
            .count();
        self.gate.should_replace(new_wins, total_trials, report.accuracy_delta)
    }
}
