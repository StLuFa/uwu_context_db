//! `SqliteVersionStore` —— Sqlite 后端的版本 DAG 存储。
//!
//! # 设计要点
//!
...
/// SQLite 后端 VersionStore，基于 `uwu_database::DbPool` + `sqlx` 直接访问。
///
/// # 快照缓存（三级）
///
...
impl SqliteVersionStore {
    ...
    async fn load_parents(&self, id: &CommitId) -> Result<Vec<CommitId>> {
        let rows = sqlx::query(
            r#"SELECT parent_id FROM version_commit_parents
               WHERE commit_id = ? ORDER BY ordinal ASC"#,
        )
        .bind(id.0)
    ...
    async fn first_parent(&self, id: &CommitId) -> Result<Option<CommitId>> {
        let row = sqlx::query(
            r#"SELECT parent_id FROM version_commit_parents
               WHERE commit_id = ? ORDER BY ordinal ASC LIMIT 1"#,
        )
        .bind(id.0)
        .fetch_optional(self.sqlite())
        .await
    ...
    async fn commit_timestamp(&self, id: &CommitId) -> Result<Option<DateTime<Utc>>> {
        let row = sqlx::query("SELECT timestamp FROM version_commits WHERE id = ?")
            .bind(id.0)
            .fetch_optional(self.sqlite())
            .await
            .map_err(Self::storage_err)?;
    ...
    async fn load_deltas(&self, id: &CommitId) -> Result<Vec<DeltaRow>> {
        let rows = sqlx::query(
            r#"SELECT uri, op, entry_json, rename_from
               FROM version_entry_deltas WHERE commit_id = ?"#,
        )
        .bind(id.0)
        .fetch_all(self.sqlite())
        .await
    ...
    async fn checkpoint_get(&self, id: &CommitId) -> Option<HashMap<String, String>> {
        let row = sqlx::query(
            r#"SELECT snapshot_json FROM version_commit_checkpoints WHERE commit_id = ?"#,
        )
        .bind(id.0)
        .fetch_optional(self.sqlite())
        .await
    ...
    async fn checkpoint_prune(&self) {
        ...
        if let Err(error) = sqlx::query(
            ...
            WITH protected_refs AS (
                ...
                LIMIT GREATEST((SELECT max(total_rows) FROM ranked) - ?, 0)
            )
    ...
    async fn require_commit_scope(&self, id: &CommitId, scope: &str) -> Result<()> {
        let actual: Option<String> =
            sqlx::query_scalar("SELECT scope FROM version_commits WHERE id = ?")
                .bind(id.0)
                .fetch_optional(self.sqlite())
                .await
                .map_err(Self::storage_err)?;
        match actual {
            Some(actual) if actual == scope => Ok(()),
            Some(_) => Err(VersionError::NotFound(format!(
                "commit {} in scope {scope}",
                id.0
            ))),
            None => Err(VersionError::NotFound(format!("commit {}", id.0))),
        }
    }
    ...
    async fn get_head(&self, scope: &str) -> Result<Option<CommitId>> {
        let row = sqlx::query("SELECT commit_id FROM version_heads WHERE scope = ?")
            .bind(scope)
            .fetch_optional(self.sqlite())
            .await
            .map_err(Self::storage_err)?;
    ...
    async fn advance_head_if_matches(
        ...
        let affected =
            sqlx::query("UPDATE version_heads SET commit_id = ? WHERE scope = ? AND commit_id = ?")
                .bind(commit.0)
                .bind(scope)
                .bind(expected.0)
                .execute(&mut **tx)
    ...
    async fn get_branch(&self, scope: &str, name: &BranchName) -> Result<Option<Branch>> {
        let row = sqlx::query(
            r#"SELECT name, head, branch_type, lifecycle_json, created_from, created_at
               FROM version_branches WHERE scope = ? AND name = ?"#,
        )
        .bind(scope)
        .bind(name.as_str())
        .fetch_optional(self.sqlite())
    ...
    async fn update_branch_head(
        ...
        let affected = sqlx::query(
            "UPDATE version_branches SET head = ? WHERE scope = ? AND name = ? AND head = ?",
        )
        .bind(head.0)
        .bind(scope)
        .bind(name.as_str())
...
/// 把 delta 应用到快照上（就地修改）。
fn apply_deltas(snapshot: &mut HashMap<String, String>, deltas: &[DeltaRow]) -> Result<()> {
    for d in deltas {
        match d.op.as_str() {
            "add" | "update" => {
                let v = d.entry_json.as_ref().ok_or_else(|| {
                    VersionError::Storage(format!("{} delta for {} has no entry_json", d.op, d.uri))
                })?;
            ...
            other => return Err(VersionError::Storage(format!("unknown delta op {other}"))),
        }
    }
    Ok(())
}

fn branch_from_row(row: sqlx::sqlite::SqliteRow) -> Result<Branch> {
    let name: String = row
        .try_get("name")
...
impl VersionStore for SqliteVersionStore {
    ...
    async fn commit(
        ...
        // SQLite 写事务串行化写入；随后读取 HEAD 并以条件更新实现 CAS，防止静默覆盖。
        let head_row =
            sqlx::query("SELECT commit_id, attached_branch FROM version_heads WHERE scope = ?")
                .bind(&scope_key)
                .fetch_optional(&mut *tx)
                .await
                .map_err(Self::storage_err)?;
        let locked_parent = head_row
            .as_ref()
            .map(|row| row.try_get::<Uuid, _>("commit_id"))
            .transpose()
            .map_err(Self::storage_err)?
            .map(CommitId);
        let attached: Option<String> = head_row
            .as_ref()
            .map(|row| row.try_get::<Option<String>, _>("attached_branch"))
            .transpose()
            .map_err(Self::storage_err)?
            .flatten();
        if locked_parent != parent {
            return Err(VersionError::Storage(format!(
        ...
        if let Some(expected) = &parent {
            if let Some(branch) = attached.as_deref() {
                let affected = sqlx::query("UPDATE version_branches SET head = ? WHERE scope = ? AND name = ? AND head = ?")
                    .bind(commit_id.0).bind(&scope_key).bind(branch).bind(expected.0)
                    .execute(&mut *tx).await.map_err(Self::storage_err)?;
                if affected.rows_affected() != 1 {
                    return Err(VersionError::Storage(format!(
                        "attached branch {branch} changed concurrently"
                    )));
                }
            }
            Self::update_head_in_tx(&mut tx, &scope_key, expected, &commit_id).await?;
        } else {
            // A new scope starts detached; callers explicitly attach it with switch_head.
            sqlx::query(
                "INSERT INTO version_heads (scope, commit_id, attached_branch) VALUES (?, ?, NULL)",
            )
            .bind(&scope_key)
            .bind(commit_id.0)
            .execute(&mut *tx)
    ...
    async fn create_branch(
        ...
        self.require_commit_scope(&from, &scope_key).await?;
        let now = Utc::now();
        sqlx::query(
    ...
    async fn list_branches(&self, scope: &ContextUri) -> Result<Vec<Branch>> {
        ...
        let rows = sqlx::query(
            r#"SELECT name, head, branch_type, lifecycle_json, created_from, created_at
               FROM version_branches WHERE scope = ? ORDER BY created_at ASC"#,
        )
        .bind(&scope_key)
        .fetch_all(self.sqlite())
        .await
    ...
    async fn delete_branch(&self, scope: &ContextUri, name: &BranchName) -> Result<()> {
        let scope_key = Self::scope_key(scope);
        sqlx::query("DELETE FROM version_branches WHERE scope = ? AND name = ?")
            .bind(&scope_key)
            .bind(name.as_str())
            .execute(self.sqlite())
            .await
    ...
    async fn switch_head(&self, scope: &ContextUri, branch: &BranchName) -> Result<()> {
        ...
        sqlx::query("UPDATE version_heads SET commit_id = ?, attached_branch = ? WHERE scope = ?")
            .bind(b.head.0)
            .bind(branch.as_str())
            .bind(&scope_key)
            .execute(self.sqlite())
    ...
    async fn create_tag(&self, scope: &ContextUri, tag: Tag) -> Result<()> {
        let scope_key = Self::scope_key(scope);
        self.require_commit_scope(&tag.target, &scope_key).await?;
    ...
    async fn read_at(
        ...
        let commit_id = match ref_ {
            ...
            VersionRef::Tag(name) => {
                let row =
                    sqlx::query("SELECT target FROM version_tags WHERE scope = ? AND name = ?")
                        .bind(&scope_key)
                        .bind(name.as_str())
                        .fetch_optional(self.sqlite())
                        .await
    ...
    async fn diff_commits(
        ...
        let scope_key = Self::scope_key(_scope);
        self.require_commit_scope(a, &scope_key).await?;
        self.require_commit_scope(b, &scope_key).await?;
    ...
    async fn squash(
        ...
        let newest = commits
            .last()
            .ok_or_else(|| VersionError::Storage("empty squash".into()))?
            .clone();
        let oldest = commits
            .first()
            .ok_or_else(|| VersionError::Storage("empty squash".into()))?
        ...
        for commit in &commits {
            self.require_commit_scope(commit, &scope_key).await?;
        }
        for pair in commits.windows(2) {
            if self.first_parent(&pair[1]).await? != Some(pair[0].clone()) {
                return Err(VersionError::Storage("squash commits must be a consecutive first-parent range ordered oldest to newest".into()));
            }
        }
        ...
        let branches =
            sqlx::query("SELECT name FROM version_branches WHERE scope = ? AND head = ?")
                .bind(&scope_key)
                .bind(newest.0)
                .fetch_all(&mut *tx)
                .await
                .map_err(Self::storage_err)?;
        let head_matches: i64 = sqlx::query_scalar(
            "SELECT count(*) FROM version_heads WHERE scope = ? AND commit_id = ?",
        )
        .bind(&scope_key)
        .bind(newest.0)
        .fetch_one(&mut *tx)
        .await
        .map_err(Self::storage_err)?;
        if branches.is_empty() && head_matches == 0 {
            return Err(VersionError::Storage(
                "squash tip is not referenced by a branch or HEAD".into(),
            ));
        }
        for row in branches {
            let name: String = row.try_get("name").map_err(Self::storage_err)?;
            ...
            if changed.rows_affected() != 1 {
                return Err(VersionError::Storage("squash branch CAS failed".into()));
            }
        }
        sqlx::query("UPDATE version_heads SET commit_id = ? WHERE scope = ? AND commit_id = ?")
            .bind(new_id.0)
    ...
    async fn gc(&self, scope: &ContextUri, policy: &GcPolicy) -> Result<GcReport> {
        ...
        let rows = sqlx::query(
            ...
            WITH RECURSIVE protected_refs AS (
                SELECT commit_id AS id FROM version_heads WHERE scope = ?
                UNION
                SELECT head AS id FROM version_branches WHERE scope = ?
                UNION
                SELECT target AS id FROM version_tags WHERE scope = ?
            ), protected AS (
        ...
        let mut tx = self.sqlite().begin().await.map_err(Self::storage_err)?;
        for id in &ids {
            sqlx::query("DELETE FROM version_commits WHERE id = ? AND scope = ?")
                .bind(id)
                .bind(&scope_key)
                .execute(&mut *tx)
                .await
    ...
    async fn evaluate_semantic_tags(&self, scope: &ContextUri) -> Result<Vec<(TagName, CommitId)>> {
        ...
        let tag_rows = sqlx::query(
            r#"SELECT name, condition_expr FROM version_tags
               WHERE scope = ? AND tag_type = 'Semantic'
    ...
    async fn impact_analysis(&self, commit: &CommitId) -> Result<ImpactAnalysis> {
        ...
        let rows =
            sqlx::query(r#"SELECT DISTINCT uri FROM version_entry_deltas WHERE commit_id = ?"#)
                .bind(commit.0)
                .fetch_all(self.sqlite())
                .await
                .map_err(Self::storage_err)?;
        let downstream_uris: Vec<ContextUri> = rows
            ...
            .collect();
        // 找到含此 commit 的分支
        let branch_rows = sqlx::query(r#"SELECT name FROM version_branches WHERE head = ?"#)
            .bind(commit.0)
            .fetch_all(self.sqlite())
            .await
            .map_err(Self::storage_err)?;
...
// ===========================================================================
// 私有辅助（SqliteVersionStore 上不便加为 trait 方法的）
// ===========================================================================
...
impl SqliteVersionStore {
    ...
    async fn build_cherry_pick_session(
        ...
        let scope_key = Self::scope_key(scope);
        self.require_commit_scope(commit, &scope_key).await?;
        self.require_commit_scope(target, &scope_key).await?;
    ...
    async fn commit_cherry_pick_snapshot(
        ...
        if let Some(b) = onto_branch {
            self.update_branch_head(&mut tx, &scope_key, b, target, &new_id)
                .await?;
            sqlx::query("UPDATE version_heads SET commit_id = ? WHERE scope = ? AND commit_id = ? AND attached_branch = ?")
                .bind(new_id.0).bind(&scope_key).bind(target.0).bind(b.as_str())
                .execute(&mut *tx).await.map_err(Self::storage_err)?;
        }
        tx.commit().await.map_err(Self::storage_err)?;
        Ok(new_id)
...
#[async_trait]
impl InteractiveVersionStore for SqliteVersionStore {
...
mod tests {
    ...
    async fn store(url: String) -> SqliteVersionStore {
    ...
    fn changes(text: &str) -> ChangeSet {
        ChangeSet {
            adds: vec![ContextEntry::new_text(scope(), TenantId(Uuid::nil()), text)],
            ..Default::default()
        }
    }
    async fn initial(s: &SqliteVersionStore, text: &str) -> CommitId {
        s.commit(&scope(), changes(text), CommitMeta::default())
            .await
            .unwrap()
    }

    #[tokio::test]
    async fn restart_persistence_and_time_travel() {
        let path = std::env::temp_dir().join(format!("uwu-version-{}.db", Uuid::new_v4()));
        let url = format!("sqlite://{}?mode=rwc", path.display());
        let first = {
            let s = store(url.clone()).await;
            initial(&s, "one").await
        };
        let s = store(url).await;
        assert_eq!(
            s.read_at(&scope(), VersionRef::Commit(first), ContentLevel::L0)
                .await
                .unwrap()
                .sparse_text(),
            "one"
    ...
    async fn attached_commit_advances_branch() {
        ...
        s.create_branch(&scope(), main.clone(), first, BranchType::Main)
            .await
            .unwrap();
        s.switch_head(&scope(), &main).await.unwrap();
        let second = s
            .commit(&scope(), changes("two"), CommitMeta::default())
            .await
            .unwrap();
        assert_eq!(
            s.get_branch(&scope().to_string(), &main)
                .await
                .unwrap()
                .unwrap()
                .head,
    ...
    #[tokio::test]
    async fn three_way_delete_propagates() {
        let mut snap = HashMap::from([("a".into(), "1".into())]);
        apply_deltas(
            &mut snap,
            &[DeltaRow {
                uri: "a".into(),
    ...
    async fn concurrent_commits_are_serialized() {
        let s = Arc::new(store("sqlite::memory:".into()).await);
        initial(&s, "one").await;
        let commit_scope = scope();
        let (a, b) = tokio::join!(
            s.commit(&commit_scope, changes("a"), CommitMeta::default()),
            s.commit(&commit_scope, changes("b"), CommitMeta::default())
        );
        assert_eq!(usize::from(a.is_ok()) + usize::from(b.is_ok()), 1);
    }
    #[tokio::test]
    async fn tag_and_asof_resolve() {
        let s = store("sqlite::memory:".into()).await;
        ...
        .await
        .unwrap();
        assert_eq!(s.list_tags(&scope()).await.unwrap()[0].target, id);
    }
    #[tokio::test]
    async fn squash_updates_referenced_tip() {
        ...
        let second = s
            ...
            .await
            .unwrap();
        let out = s.squash(&scope(), vec![first, second], "s").await.unwrap();
        assert_eq!(
            s.get_branch(&scope().to_string(), &main)
                .await
                .unwrap()
    ...
    #[tokio::test]
    async fn reachable_gc_preserves_head() {
        let s = store("sqlite::memory:".into()).await;
        let id = initial(&s, "one").await;
        let report = s
            .gc(
            ...
            .await
            .unwrap();
        assert_eq!(report.removed_commits, 0);
        assert!(s.load_commit(&id).await.unwrap().is_some());
    }
    #[tokio::test]
    async fn persisted_conflict_session_resumes() {
        ...
        s.create_branch(&scope(), main.clone(), id.clone(), BranchType::Main)
            ...
            .unwrap();
        let session = s
            .begin_cherry_pick(&scope(), &id, &main, ConflictSessionPersistence::Enabled)
            .await
            .unwrap();
        assert_eq!(
            s.load_conflict_session(&session.id).await.unwrap().id,
            session.id
        );
        s.abort_conflict_session(&session.id).await.unwrap();
        assert!(s.load_conflict_session(&session.id).await.is_err());
    }
}
